...
- **What is checked** — both externally-verifiable claims (a CLI flag, an API signature, library behaviour, and the cited spec's caveats and scope limits, not just its headline rule) **and** repo-grounded ones (a named path that does not exist, a description of current behaviour that contradicts the code, an approach infeasible against the existing implementation). **Findings are bounded to factual/feasibility defects** — the approach is wrong or infeasible — never to implementation guidance (flag choices, edge-case handling, fixture design, output formatting): that class is decided later, against the live repository, not at validation time. This boundary is owned by `agents/issue-validator.md` itself (caller-agnostic, §2), so it applies wherever the agent runs, including its `implement-issue` invocation below.
...
- **Findings handling** — because nothing is posted yet, a `severity: error` blocker does **not** stop the run: the skill folds the validator's prescribed correction back into a re-draft and re-validates **only the claims that changed** — derived from a diff against the immediately-prior draft, never the whole draft again — capped at two rounds. The same delta-only re-validation applies to the developer's own change-request loop at the review gate, so a full re-validation never runs twice inside one `create-issue` invocation. A single validation response can carry an error and warnings together, so the skill keeps a **pending list** of every `warning` finding across every round of the *current* error-correction pass — the round that started it and each correction round — rather than only the most recent delta round's output, so a warning that arrived alongside a blocker is never dropped just because the round that follows never re-surfaces it. `warning` findings never reach the developer for adjudication: once that pass's error-correction loop concludes, the skill folds its pending list directly into the draft's `## References` section — each note tagged with its `issue-validator` provenance — before the gate. Because `issue-writer` regenerates the body from scratch on every call, any note an **earlier** pass already folded is not preserved by regeneration on its own, so every re-invocation of `issue-writer` (by this correction loop or by the change-request loop below) re-appends the exact References content already committed by an earlier fold onto the fresh draft before anything else — carry-forward of already-committed text, kept distinct from the pending list so a note is never folded twice. Only a residual blocker surviving the two-round cap reaches the draft-review human gate.
