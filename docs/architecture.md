...
- **Builder** — edit files, write tests, run commands. Read + write, project-confined. Given a plan, a builder implements it as decided; given a set of **review findings**, it also triages them — deciding which are worth applying, bounded by severity (`error` applies by default, `warning` is discretionary) and reporting applied-vs-skipped with reasons. Triage is scoped to the findings it was handed; it never widens the work.
...
**The review tail, and why there are two fix-owners.** After verification passes, `implement-issue` continues into a **deep review**: the three review agents run in parallel over the implemented change, a builder agent triages and applies their findings, and the verifier re-runs. This review sits **here**, in `implement-issue`, and not in `check-out` — a deliberate reversal of the original sequencing. Placed in `check-out`, the automated review ran *after* the developer's manual review at interception 2, so the fixes it produced changed code the developer had already signed off, forcing them to re-review and re-test it. Reviewing before the hand-off means the developer's manual review lands on already-reviewed-and-fixed code. It also sharpens the per-skill responsibilities: `implement-issue` owns *producing correct, clean, reviewed code*; `check-out` owns *confirming and shipping it* (§5).
...
- **No fix loop, no retry — failures STOP and escalate.** Unlike `implement-issue`, `check-out` never fixes-and-continues and never retries. Any blocker halts the run and escalates to the developer, who resolves it and re-runs `check-out`:
...
- **`implementation-verifier` (scoped, in-loop).** Runs a **targeted subset** of the test suite — the tests covering the changed area/module (e.g. the affected Django app, or the affected CDK service/resource), not merely the changed test files and not the whole suite. It runs inside **both** of `implement-issue`'s fix loops (up to 3× each), so a scoped run keeps them fast and cheap.
- **Pre-merge validation (full, at the gate).** Runs the project's **full** test/lint suite once at `check-out` — the final safety net that catches any cross-module regression a scoped run could not see. It is the only verification `check-out` runs; it no longer applies fixes, so there is no scoped run there.
...
**In `implement-issue` (implementation time, against the posted issue).** The skill posts **only** the validator's actual findings as a GitHub issue comment — one line each, and no comment at all when there are none (never the verdict or a recap of confirmed claims). When any finding is `severity: error`, the skill then **stops**; the developer must update the issue before re-running `implement-issue`. `warning` findings are posted (terse) and carried into the implementation plan.
