...
3. **Plan** (skill, main thread) — `EnterPlanMode`: in one read-only context, explore the codebase and produce a **concrete** plan (exact files and edits, test strategy), folding in any minor findings. The plan also **sweeps**: a fact, rule, or name it changes is located in every place the repo states it and all are folded in, so building can't leave the repo self-contradicting — the same obligation the builder agent carries at step 9, applied here because the main thread authors the plan's edits. Nothing is written yet.
...
|   factual/consistency defect -> apply at     |          |
|   any severity; other warning -> discretion. |          |
|   Report applied vs skipped -> developer     |          |
+----------------------------------------------+          |
     |                                                    |
     v                                                    |
+----------------------------------------------+          |
